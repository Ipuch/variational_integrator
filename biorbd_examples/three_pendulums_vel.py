...
def three_pendulums(time: float = 20, time_step: float = 0.05, unit_test = False):
    ...
    if unit_test:
        ...
        for i in range(3):
            for j in range(2):
                axs[i, j].plot(
                    ...
                    q_vi[2 * i + j, :],
                    label="Variational Integrator with two initial states"
                )
                axs[i, j].plot(
                    np.arange(0, time, time_step),
                    q_vi_vel[2 * i + j, :],
        ...
        plt.figure()
        plt.plot(fcn_constraint(q_vi_vel).toarray()[0, :], label="constraint0")
        plt.plot(fcn_constraint(q_vi_vel).toarray()[1, :], label="constraint1")
        plt.plot(fcn_constraint(q_vi_vel).toarray()[2, :], label="constraint2")
        plt.plot(fcn_constraint(q_vi_vel).toarray()[3, :], label="constraint3")
