...
class VariationalIntegrator:
    ...
    def __init__(
        self,
        biorbd_model: bionc.BiomechanicalModel,
        nb_steps: int,
        time: float,
        q_init: np.ndarray,
        ...
        self.biomodel = biorbd_model
        # self.biorbd_numpy_model = biorbd.Model(biorbd_model.path().absolutePath().to_string())
        self.time = time
        self.nb_steps = nb_steps
        self.time_step = time / nb_steps

        self.constraints = self.biomodel.holonomic_constraints
        self.jac = self.biomodel.holonomic_constraints_jacobian
        ...
        if initial_guess_approximation == InitialGuessApproximation.CUSTOM:
            ...
            if initial_guess_custom.shape != (self.biomodel.nb_Q, self.nb_steps):
                raise ValueError(
                    ...
                    f"{initial_guess_custom.shape} and it should be ({self.biomodel.nb_Q}, "
                    f"{self.nb_steps})"
                )
        ...
        if controls is None:
            self.controls = np.zeros((self.biomodel.nb_Q, self.nb_steps))
    ...
    def _compute_initial_states(self, q_init: np.ndarray, q_dot_init: np.ndarray) -> Tuple[DM, DM, DM | None]:
        ...
        q0 = NaturalCoordinates(MX.sym("q0", self.biomodel.nb_Q, 1))
        q0_dot = NaturalCoordinates(MX.sym("q0_dot", self.biomodel.nb_Q, 1))
        q1 = NaturalCoordinates(MX.sym("q1", self.biomodel.nb_Q, 1))
        f0_minus = NaturalCoordinates(MX.sym("f0_minus", self.biomodel.nb_Q, 1))
        ...
        constraint_term = (
            ...
            else MX.zeros(self.biomodel.nb_Q, 1)
        )
        ...
        v_init = self._dispatch_to_v(q_init + q_dot_init * self.time_step + 0.0001, lambdas_num)
        v_opt = ifcn(v_init)
    ...
    def _compute_final_velocity(self, q_penultimate: np.ndarray, q_ultimate: np.ndarray) -> Tuple[DM | None, DM | None]:
        ...
        if (
            ...
            # Declare the MX variables
            qN = NaturalCoordinates(MX.sym("qN", self.biomodel.nb_Q, 1))
            qN_dot = NaturalCoordinates(MX.sym("qN_dot", self.biomodel.nb_Q, 1))
            qN_minus_1 = NaturalCoordinates(MX.sym("qN_minus_1", self.biomodel.nb_Q, 1))
            fd_plus = NaturalCoordinates(MX.sym("fd_plus", self.biomodel.nb_Q, 1))
    ...
    def _declare_mx(self):
        ...
        self.control_prev = MX.sym("control_prev", self.biomodel.nb_Q, 1)
        self.control_cur = MX.sym("control_cur", self.biomodel.nb_Q, 1)
        self.control_next = MX.sym("control_next", self.biomodel.nb_Q, 1)
    ...
    def lagrangian(self, q: MX | SX, qdot: MX | SX) -> MX | SX:
        ...
        return self.biomodel.lagrangian(q, qdot)
